...
def _claude_effort():                                                 # claude -p 的推理 effort(low/medium/high);空=不传,用 claude 默认
    e = (_model_prefs().get("claude-code__effort") or "").strip().lower()
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/model-pref":            # 各 provider 当前所选模型(含默认),前端画下拉用
            ...
            return self._json({"prefs": out, "effort": {"claude-code": _claude_effort()}})
        if p == "/api/pins":
            if not self._authed():
                return self._deny()
            try:
    ...
    def do_POST(self):
        ...
        if p == "/api/model-pref":   # 设置某 provider 选用的模型变体(单窗口 + 对比共用)
            ...
            if "effort" in data and prov == "claude-code":        # 存 claude effort(空字符串=清掉,用默认)
                _set_model_pref("claude-code__effort", effort if effort in ("low","medium","high") else "")
            # claude-code 的模型/effort 走 env(后端启动时定),改了要重启它的后端;其它 provider 是 thread 级钉,新对话自然生效
            restarted = False
            if prov == "claude-code":
                port = CMP_PORTS.get(prov)
                if port:
